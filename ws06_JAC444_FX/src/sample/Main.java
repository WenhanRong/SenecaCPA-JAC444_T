...
import static sample.Chessboard.initChessboard;
...
public class Main extends Application {
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        primaryStage.setScene(new Scene(root, CONSTANT.BOARD_DIMENSION, CONSTANT.BOARD_DIMENSION));
        ...
        Scene scene = new Scene(gpane, CONSTANT.BOARD_DIMENSION, CONSTANT.BOARD_DIMENSION);
    ...
    void manualStartup(GridPane gpane, Knight knight) throws Exception {
        ...
        gpane.addEventHandler(MouseEvent.MOUSE_CLICKED, e -> {
            ...
            for (int i = 0; i < knight.getMoveNumberIcon().length; i++) {
                try {
                    if (!matched) {
                        if (clickX != knight.getCurrentColumn() && clickY != knight.getCurrentRow()) {
    ...
    Knight startupWindow(GridPane gpane) throws Exception {
        ...
        startupStage.setScene(new Scene(root1, CONSTANT.BOARD_DIMENSION - 50, 250));
        ...
        startupStage.setScene(new Scene(startupPane, CONSTANT.BOARD_DIMENSION - 50, 200));
        startupStage.show();
    ...
    void initStartupWindow(Stage startupStage, BorderPane startupPane, GridPane gpane, Knight knight) {
        ...
        start.setOnAction(actionEvent -> {
            try {
                ...
                if (posX >= 0 && posX <= CONSTANT.SQUARE_ROW_TOTAL - 1) {
                    if (posY >= 0 && posY <= CONSTANT.SQUARE_ROW_TOTAL - 1) {
                        knight.setCurrentColumn(posX);
                        knight.setCurrentRow(posY);
                ...
                if (!validInput) {
                    pane.getChildren().add(setErrorMessage("Invalid input! Please choose a value from 0 to " + (CONSTANT.SQUARE_ROW_TOTAL - 1), new Insets(30, 0, 0, 85)));
                }
